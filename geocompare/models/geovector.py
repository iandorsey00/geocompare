...
class GeoVector:
    ...
    def __init__(self, db_row, medians, standard_deviations):
        ...
        self.d["average_household_size"] = db_row["B25010_1"]
        self.d["occupied_housing_units"] = db_row["B25003_1"]
        self.d["homeowner_occupied_housing_units"] = db_row["B25003_2"]
        self.d["median_rooms"] = db_row["B25018_1"]
        ...
        def gdsdd(dividend, divisor, numeric_dividend=False):
            """Wrapper for safe_divide for GeoVector data."""
            if not numeric_dividend:
                return safe_divide(self.d[dividend], self.d[divisor])
            else:
                return safe_divide(dividend, self.d[divisor])

        self.rs["population_density"] = gdsdd("population", "land_area_sqmi")
        self.rs["per_capita_income"] = parse_int(self.d["per_capita_income"])
        ...
        self.rs["bachelors_degree_or_higher"] = (
            gdsdd(
                parse_int(self.d["bachelors_degree"])
                + parse_int(self.d["masters_degree"])
                + parse_int(self.d["professional_school_degree"])
                + parse_int(self.d["doctorate_degree"]),
                "population_25_years_or_older",
                numeric_dividend=True,
            )
            * 100.0
        )

        self.rs["graduate_degree_or_higher"] = (
            gdsdd(
                parse_int(self.d["masters_degree"])
                + parse_int(self.d["professional_school_degree"])
                + parse_int(self.d["doctorate_degree"]),
        ...
        self.rs["housing_density"] = gdsdd("occupied_housing_units", "land_area_sqmi")
        self.rs["average_household_size"] = parse_float(self.d["average_household_size"])
        self.rs["homeowner_occupied_housing_units"] = (
            gdsdd("homeowner_occupied_housing_units", "occupied_housing_units") * 100.0
        )
        self.rs["median_year_structure_built"] = (
            parse_int(self.d["median_year_structure_built"]) - 1939
        )
        self.rs["median_rooms"] = parse_float(self.d["median_rooms"])
        ...
        def gdsdm(dividend, divisor, numeric_dividend=False):
            ...
            if not numeric_dividend:
                return safe_divide(float(medians[dividend]), float(medians[divisor]))
            else:
                return safe_divide(dividend, float(medians[divisor]))

        self.med["population_density"] = gdsdm("B01003_1", "ALAND_SQMI")
        self.med["per_capita_income"] = parse_float(medians["B19301_1"])
        self.med["white_alone"] = gdsdm("B02001_2", "B01003_1") * 100.0
        self.med["black_alone"] = gdsdm("B02001_3", "B01003_1") * 100.0
        self.med["asian_alone"] = gdsdm("B02001_5", "B01003_1") * 100.0
        self.med["hispanic_or_latino"] = gdsdm("B03002_12", "B01003_1") * 100.0

        self.med["bachelors_degree_or_higher"] = (
            gdsdm(
                parse_int(medians["B15003_22"])
                + parse_int(medians["B15003_23"])
                + parse_int(medians["B15003_24"])
                + parse_int(medians["B15003_25"]),
                "B15003_1",
                numeric_dividend=True,
            )
            * 100.0
        )
        self.med["graduate_degree_or_higher"] = (
            gdsdm(
                parse_int(medians["B15003_23"])
                + parse_int(medians["B15003_24"])
                + parse_int(medians["B15003_25"]),
        ...
        self.med["housing_density"] = gdsdm("B25003_1", "ALAND_SQMI")
        self.med["average_household_size"] = parse_float(medians["B25010_1"])
        self.med["homeowner_occupied_housing_units"] = gdsdm("B25003_2", "B25003_1") * 100.0
        self.med["median_year_structure_built"] = parse_float(medians["B25035_1"]) - 1939
        self.med["median_rooms"] = parse_float(medians["B25018_1"])
        ...
        def gdsds(dividend, divisor, numeric_dividend=False):
            ...
            if not numeric_dividend:
                return safe_divide(
                    float(standard_deviations[dividend]),
                    float(standard_deviations[divisor]),
                )
            else:
                return safe_divide(dividend, float(standard_deviations[divisor]))

        self.sd["population_density"] = gdsds("B01003_1", "ALAND_SQMI")
        self.sd["per_capita_income"] = parse_float(standard_deviations["B19301_1"])
        self.sd["white_alone"] = gdsds("B02001_2", "B01003_1") * 100.0
        self.sd["black_alone"] = gdsds("B02001_3", "B01003_1") * 100.0
        self.sd["asian_alone"] = gdsds("B02001_5", "B01003_1") * 100.0
        self.sd["hispanic_or_latino"] = gdsds("B03002_12", "B01003_1") * 100.0

        self.sd["bachelors_degree_or_higher"] = (
            gdsds(
                parse_int(standard_deviations["B15003_22"])
                + parse_int(standard_deviations["B15003_23"])
                + parse_int(standard_deviations["B15003_24"])
                + parse_int(standard_deviations["B15003_25"]),
                "B15003_1",
                numeric_dividend=True,
            )
            * 100.0
        )
        self.sd["graduate_degree_or_higher"] = (
            gdsds(
                parse_int(standard_deviations["B15003_23"])
                + parse_int(standard_deviations["B15003_24"])
                + parse_int(standard_deviations["B15003_25"]),
        ...
        self.sd["housing_density"] = gdsds("B25003_1", "ALAND_SQMI")
        self.sd["average_household_size"] = parse_float(standard_deviations["B25010_1"])
        self.sd["homeowner_occupied_housing_units"] = gdsds("B25003_2", "B25003_1") * 100.0
        self.sd["median_year_structure_built"] = parse_float(standard_deviations["B25035_1"])
        self.sd["median_rooms"] = parse_float(standard_deviations["B25018_1"])
        ...
        # Standard mode
        self.ws["std"] = dict()
        # Built-form mode. Kept under "app" for compatibility.
        self.ws["app"] = dict()
        ...
        #
        # The built-form component:
        #   Built-form mode only:
        #     Population density (25%)
        #     Housing density (20%)
        #     Owner occupancy (20%)
        #     Median year structure built (15%)
        #     Median rooms (10%)
        #     Average household size (10%)
        #
        self.ws["app"]["population_density"] = self.s["population_density"] * 0.25
        self.ws["app"]["housing_density"] = self.s["housing_density"] * 0.20
        self.ws["app"]["homeowner_occupied_housing_units"] = (
            self.s["homeowner_occupied_housing_units"] * 0.20
        )
        self.ws["app"]["median_year_structure_built"] = self.s["median_year_structure_built"] * 0.15
        self.ws["app"]["median_rooms"] = self.s["median_rooms"] * 0.10
        self.ws["app"]["average_household_size"] = self.s["average_household_size"] * 0.10
        self.ws["form"] = self.ws["app"]
    ...
    def __repr__(self):
        ...
        return (
            ...
            f"population={parse_int(self.d['population'])}, "
